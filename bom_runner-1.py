...
from bom_layout import detect_layout, validate_layout, __version__, __fecha__
...
def process_one(pdf_path, paquete, output_folder, generar_limpio=True):
    ...
    # 2) Rev Description del BOM. Se aplica SIEMPRE que se indique en Config,
    #    aunque 'agregar_bom' sea NO: son cosas independientes. Si el BOM ya
    #    está en la lista, va ANTES de tocar su Rev (esa operación escribe
    #    debajo y desplazaría la referencia).
    ya_estaba = False
    if cfg.get("codigo_bom"):
    ...
    # 3) Agregar BOM al inicio (condicional)
...
def run_batch(input_folder, xlsx_path, output_folder, generar_limpio=True):
    ...
    print(f"Redline de BOMs v{__version__} ({__fecha__})")
