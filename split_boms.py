...
def localizar_boms(doc):
    ...
    # Quitar duplicados contiguos (el mismo item repetido en la misma posición)
    limpio = []
    for e in encontrados:
        if limpio and e["item"] == limpio[-1]["item"] \
                and e["pagina"] == limpio[-1]["pagina"] \
                and abs(e["y_inicio"] - limpio[-1]["y_inicio"]) < 3:
            continue
        limpio.append(e)
    return limpio
...
def main():
    ...
    pdfs = sorted(list(entrada.glob("*.pdf")) + list(entrada.glob("*.PDF")))
    if not pdfs:
